...
request.setAttribute("pageTitle", "Browse skills"); CategoryDao categoryDao =
new CategoryDao(); SkillPostDao postDao = new SkillPostDao(); List<Category>
  categories = categoryDao.findAll(); List<SkillPost>
    posts = postDao.latest(12); %> <%@ include file="/layouts/main_top.jsp" %>
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-10">
      <div class="bg-white rounded-3xl border border-slate-100 shadow-xl p-8">
        ...
        <form
          action="<%= request.getContextPath() %>/search.jsp"
          method="GET"
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <% } %> <% if (posts.isEmpty()) { %>
        <div
          class="bg-white rounded-3xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-center py-16"
        >
          <img
